...
void thread_func1(const Log::Logger<DefaultDataProvider, ConsoleSink> &log) {
...
void thread_func2(const Log::Logger<DefaultDataProvider, ConsoleSink> &log) {
    for (int i = 0; i < 1000; i++) {
        Error(log, "thread 2\n");  // simple log
    }
}
...
int main() {
    ...
    auto thread1 = std::thread(thread_func1, myLogger);
    auto thread2 = std::thread(thread_func2, myLogger);
    thread1.join();
    thread2.join();
